...
from qiskit_calculquebec.API.api_utility import ApiUtility, keys
...
def mock_list_machines():
    with patch("qiskit_calculquebec.API.adapter.ApiAdapter.list_machines") as mock:
        yield mock
...
def mock_get_project_id_by_name():
    with patch(
        "qiskit_calculquebec.API.adapter.ApiAdapter.get_project_id_by_name"
    ) as mock:
        yield mock
...
def mock_is_last_update_expired():
    with patch(
        "qiskit_calculquebec.API.adapter.ApiAdapter.is_last_update_expired"
...
@pytest.fixture
def mock_job_body():
    with patch("qiskit_calculquebec.API.api_utility.ApiUtility.job_body") as job_body:
        return job_body
